...
public abstract class EfCoreNotificationReceiverRepositoryBase : EfCoreRepository<HCDbContext, NotificationReceiver, Guid>
{
    ...
    public virtual async Task<List<NotificationReceiverWithNavigationProperties>> GetListWithNavigationPropertiesAsync(string? filterText = null, bool? isRead = null, DateTime? readAtMin = null, DateTime? readAtMax = null, Guid? notificationId = null, Guid? identityUserId = null, string? sorting = null, int maxResultCount = int.MaxValue, int skipCount = 0, CancellationToken cancellationToken = default)
    {
        var query = await GetQueryForNavigationPropertiesAsync();
        query = ApplyFilter(query, filterText, isRead, readAtMin, readAtMax, notificationId, identityUserId);
    ...
    protected virtual IQueryable<NotificationReceiverWithNavigationProperties> ApplyFilter(IQueryable<NotificationReceiverWithNavigationProperties> query, string? filterText, bool? isRead = null, DateTime? readAtMin = null, DateTime? readAtMax = null, Guid? notificationId = null, Guid? identityUserId = null)
    {
        return query.WhereIf(!string.IsNullOrWhiteSpace(filterText), e => true).WhereIf(isRead.HasValue, e => e.NotificationReceiver.IsRead == isRead).WhereIf(readAtMin.HasValue, e => e.NotificationReceiver.ReadAt >= readAtMin!.Value).WhereIf(readAtMax.HasValue, e => e.NotificationReceiver.ReadAt <= readAtMax!.Value).WhereIf(notificationId != null && notificationId != Guid.Empty, e => e.Notification != null && e.Notification.Id == notificationId).WhereIf(identityUserId != null && identityUserId != Guid.Empty, e => e.IdentityUser != null && e.IdentityUser.Id == identityUserId);
    }
    ...
    public virtual async Task<long> GetCountAsync(string? filterText = null, bool? isRead = null, DateTime? readAtMin = null, DateTime? readAtMax = null, Guid? notificationId = null, Guid? identityUserId = null, CancellationToken cancellationToken = default)
    {
        var query = await GetQueryForNavigationPropertiesAsync();
        query = ApplyFilter(query, filterText, isRead, readAtMin, readAtMax, notificationId, identityUserId);
