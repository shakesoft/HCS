...
public abstract class EfCoreSurveyLocationRepositoryBase : EfCoreRepository<HCDbContext, SurveyLocation, Guid>
{
    public EfCoreSurveyLocationRepositoryBase(IDbContextProvider<HCDbContext> dbContextProvider) : base(dbContextProvider)
    {
    }
    ...
    public virtual async Task<List<SurveyLocation>> GetListAsync(string? filterText = null, string? code = null, string? name = null, string? description = null, bool? isActive = null, string? sorting = null, int maxResultCount = int.MaxValue, int skipCount = 0, CancellationToken cancellationToken = default)
    {
        var query = ApplyFilter((await GetQueryableAsync()), filterText, code, name, description, isActive);
        query = query.OrderBy(string.IsNullOrWhiteSpace(sorting) ? SurveyLocationConsts.GetDefaultSorting(false) : sorting);
    ...
    protected virtual IQueryable<SurveyLocation> ApplyFilter(IQueryable<SurveyLocation> query, string? filterText = null, string? code = null, string? name = null, string? description = null, bool? isActive = null)
    {
