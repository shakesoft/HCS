...
````bash
Add-Migration Your_Migration_Name -Context HCDbContext
````

Using EF Core command line tool;

````bash
dotnet ef migrations add Your_Migration_Name --context HCDbContext
````
...
````bash
Add-Migration Your_Migration_Name -Context HCTenantDbContext -OutputDir TenantMigrations
````

Using EF Core command line tool:

````bash
dotnet ef migrations add Your_Migration_Name --context HCTenantDbContext --output-dir TenantMigrations
````
