...
public abstract class EfCoreProjectTaskRepositoryBase : EfCoreRepository<HCDbContext, ProjectTask, Guid>
{
    ...
    public virtual async Task<ProjectTaskWithNavigationProperties> GetWithNavigationPropertiesAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        return (await GetDbSetAsync()).Where(b => b.Id == id).Select(projectTask => new ProjectTaskWithNavigationProperties { ProjectTask = projectTask, Project = dbContext.Set<Project>().FirstOrDefault(c => c.Id == projectTask.ProjectId) }).FirstOrDefault();
    }
    ...
    {
        return from projectTask in (await GetDbSetAsync())
               join project in (await GetDbContextAsync()).Set<Project>() on projectTask.ProjectId equals project.Id into projects
