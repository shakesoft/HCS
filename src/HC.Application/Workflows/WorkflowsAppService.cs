...
public abstract class WorkflowsAppServiceBase : HCAppService
{
    ...
    {
        var query = (await _workflowDefinitionRepository.GetQueryableAsync()).WhereIf(!string.IsNullOrWhiteSpace(input.Filter), x => (x.Code != null && x.Code.Contains(input.Filter)) || (x.Name != null && x.Name.Contains(input.Filter)));
