using HC.Positions;
using HC.Departments;
using HC.Units;
using System;
using System.Threading.Tasks;
using Volo.Abp.Data;
using Volo.Abp.DependencyInjection;
using Volo.Abp.Uow;
using HC.AbpUsers;

namespace HC.AbpUsers;

public class AbpUsersDataSeedContributor : IDataSeedContributor, ISingletonDependency
{
    private bool IsSeeded = false;
    private readonly IAbpUserRepository _abpUserRepository;
    private readonly IUnitOfWorkManager _unitOfWorkManager;
    private readonly UnitsDataSeedContributor _unitsDataSeedContributor;
    private readonly DepartmentsDataSeedContributor _departmentsDataSeedContributor;
    private readonly PositionsDataSeedContributor _positionsDataSeedContributor;

    public AbpUsersDataSeedContributor(IAbpUserRepository abpUserRepository, IUnitOfWorkManager unitOfWorkManager, UnitsDataSeedContributor unitsDataSeedContributor, DepartmentsDataSeedContributor departmentsDataSeedContributor, PositionsDataSeedContributor positionsDataSeedContributor)
    {
        _abpUserRepository = abpUserRepository;
        _unitOfWorkManager = unitOfWorkManager;
        _unitsDataSeedContributor = unitsDataSeedContributor;
        _departmentsDataSeedContributor = departmentsDataSeedContributor;
        _positionsDataSeedContributor = positionsDataSeedContributor;
    }

    public async Task SeedAsync(DataSeedContext context)
    {
        if (IsSeeded)
        {
            return;
        }

        await _unitsDataSeedContributor.SeedAsync(context);
        await _departmentsDataSeedContributor.SeedAsync(context);
        await _positionsDataSeedContributor.SeedAsync(context);
        await _abpUserRepository.InsertAsync(new AbpUser(id: Guid.Parse("41a2de23-ddd2-4462-abfb-09e8258e0cca"), userName: "", normalizedUserName: "", name: "f55ee0ff5b894d2e9d666b1e620faa1cc1cd24aaa5614980bf489ce7dff9d5a0", surname: "1be6aa807c5c42e88e6fa1f2f109481b947eac3cbf284bec803049a625c61b16", email: "", normalizedEmail: "", emailConfirmed: true, passwordHash: "", securityStamp: "", isExternal: true, phoneNumber: "73f24c2650a940ab", phoneNumberConfirmed: true, isActive: true, twoFactorEnabled: true, lockoutEnd: new DateTime(2017, 11, 2), lockoutEnabled: true, accessFailedCount: 431773675, shouldChangePasswordOnNextLogin: true, entityVersion: 291172858, lastPasswordChangeTime: new DateTime(2003, 1, 27), unitId: null, departmentId: null, positionId: null));
        await _abpUserRepository.InsertAsync(new AbpUser(id: Guid.Parse("2b0c9752-ed28-4818-b7b3-c472cf9bc737"), userName: "", normalizedUserName: "", name: "a29069cef2b7458faba8eab60adf3d94e7040551289a45cb86bf6b4b90d6953c", surname: "9356dd8a124e405db7b55fbb9693ddc8722863df50dd41eba767a279f0783c3b", email: "", normalizedEmail: "", emailConfirmed: true, passwordHash: "", securityStamp: "", isExternal: true, phoneNumber: "423ef17da0f54c77", phoneNumberConfirmed: true, isActive: true, twoFactorEnabled: true, lockoutEnd: new DateTime(2011, 9, 8), lockoutEnabled: true, accessFailedCount: 622626696, shouldChangePasswordOnNextLogin: true, entityVersion: 1702573819, lastPasswordChangeTime: new DateTime(2006, 2, 18), unitId: null, departmentId: null, positionId: null));
        await _unitOfWorkManager!.Current!.SaveChangesAsync();
        IsSeeded = true;
    }
}