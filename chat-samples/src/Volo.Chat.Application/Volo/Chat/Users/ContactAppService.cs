...
public class ContactAppService : ChatAppService, IContactAppService
{
    ...
    public virtual async Task<List<ChatContactDto>> GetContactsAsync(GetContactsInput input)
    {
        ...
        var result = await _permissionFinder.IsGrantedAsync(conversationContacts.Select(x =>
        ...
        {
            contactDto.HasChatPermission = result.Any(x => x.UserId == contactDto.UserId && x.Permissions.All(p => p.Value));
        }
